...
def gamma_correction(Gamma_0, f_i):
    ang = Gamma_0 * 384 * np.pi ** 2 / f_i ** 2
    gamma_corrected = (np.pi ** 2 * f_i ** 2 * ang) / 117480
...
def wavelength_pattern(w, g, d, k_y):
    def f(k, w=w, g=g, d=d, k_y=k_y):
        return w ** 2 - g * (k ** 2 + k_y ** 2) ** (0.5) * np.tanh(k * d)
    k_x = fsolve(f, 0)
    k_x = k_x[0]
    wavelength_x = 2 * np.pi / k_x
    return k_x, wavelength_x

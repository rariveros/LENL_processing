...
if __name__ == "__main__":
    ...
    for k in range(N_structures):
        ...
        if delta_wind_0 % 2 != 0:
            wind_R_j = wind_R_j + 1
        print(x_grid[wind_L_j], x_grid[wind_R_j])
        delta_wind_0 = wind_R_j - wind_L_j
    ...
    np.savetxt(directory + '//I.txt', Is, delimiter=',')
    ...
    plt.grid(linestyle='--', alpha=0.5)
    plt.savefig(directory + '//crest_position.png', dpi=300)
    plt.close()
    ...
    #plt.ylim(0, 0.2)
    plt.grid(linestyle='--', alpha=0.5)
    plt.savefig(directory + '//crest_velocity.png', dpi=300)
    plt.close()
    ...
    plt.grid(linestyle='--', alpha=0.5)
    plt.savefig(directory + '//ampd_tracked.png', dpi=300)
    plt.close()
    ...
    plt.ylabel('$|A|\ (\\textrm{mm})$', size='20')
    plt.grid(linestyle='--', alpha=0.5)
    plt.savefig(directory + '//ampd(x).png', dpi=300)
    plt.close()

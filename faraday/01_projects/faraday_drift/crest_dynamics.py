...
if __name__ == '__main__':
    ...
    np.savetxt(inputs_file + '\\' + 'dvelocity_input_' + str(ampd) + '.csv', dvelocity_input_np, delimiter=',')
    np.savetxt(dvelocities_file + '\\' + 'info_dvelocity_' + str(ampd) + '.csv', info_drift_np, delimiter=',')
    #np.savetxt(inputs_file + '\\' + 'fits_dvelocity_' + str(ampd) + '.csv', fits_np, delimiter=',')

    plt.plot(fits[0][1], fits[0][0], linewidth=3, c='w')
    plt.plot(fits[1][1], fits[1][0], linewidth=3, c='w')
    plt.plot(fits[2][1], fits[2][0], linewidth=3, c='w')
    plt.plot(fits[3][1], fits[3][0], linewidth=3, c='w')
    plt.plot(fits[4][1], fits[4][0], linewidth=3, c='r')
    T_np = np.arange(len(T_per))
    T_sec = T_np / 400

    norm = TwoSlopeNorm(vmin=0, vcenter=np.amax(Z_mm) / 2, vmax=np.amax(Z_mm))
    plot = plt.pcolormesh(X_mm, T_sec, Z_mm, cmap=parula_map, norm=norm, shading='auto')
    ...
    plt.tight_layout()
    plt.savefig(carpeta + '\\' + 'drift_velocities', dpi=300)
    #plt.show()
    plt.close()

    x_fit_mm = x_fit - x_fit[0]
    x_mm = x_np - x_fit[0]

    plt.scatter(t_np[0:-1:40], x_mm[0:-1:40], c='k')
    plt.plot(t_np, x_fit_mm, linewidth=3, c='r')
    plt.xlim([t_np[0], t_np[-1]])
    plt.xlabel('$t\ \\textrm{(s)}$', size='25')
    plt.xticks(fontsize=20)
    plt.ylabel('$\Delta x\ \\textrm{(mm)}$', size='25')
    plt.yticks(fontsize=20)
    plt.grid(linestyle='--', alpha=0.2, color='k')
    plt.tight_layout()
    plt.savefig(carpeta + '\\' + 'x_drift', dpi=300)
    plt.close()
